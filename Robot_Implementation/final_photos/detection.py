...
def detect_color(path_to_image):
        ...
        #cv2.waitKey(0)
        blue =[[86, 154, 77],[111, 255, 140]] #[[84, 166, 114],[169, 248, 245]]
        green=[[43, 55, 24],[86, 220, 200]]
        red = [[135, 68, 0],[200, 255, 160]]
...
def detect_trash(path_to_image):
    kernel = np.ones((5, 5), np.uint8)
    color_range=np.array([[9,43, 93], [59, 255, 255]])
    # convert the frame to HSV co-ordinates
    frame = cv2.imread(path_to_image)
    hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
    # mask -> to apply filter to the image based on color range
...
#print(detect_trash("left_edge.jpg")